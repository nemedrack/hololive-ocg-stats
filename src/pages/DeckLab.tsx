...
export default function DeckLab() {
    ...
    const selectedDeckMeta = didToMeta.get(selectedDeck);
    const opponentDeckMeta = didToMeta.get(opponentDeck);
    const selectedDeckName = selectedDeckMeta?.name ?? selectedDeck;
    const opponentDeckName = opponentDeckMeta?.name ?? opponentDeck;

    const selectedDeckIcon = selectedDeckMeta?.icon;
    const opponentDeckIcon = opponentDeckMeta?.icon;
        ...
        <div>
            ...
            {selectedDeck && (
                <section
                    ...
                    gap: 14,
                    padding: 14,
                    borderColor: selectedDeckMeta?.color ? `${selectedDeckMeta.color}55` : undefined,
                    }}
                >
                    {selectedDeckIcon ? (
                    <img
                        src={withBase(selectedDeckIcon)}
                        alt=""
                        style={{
                        width: 72,
                        height: 72,
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                        ...
                        {selectedDeckMeta?.name ?? selectedDeck}
                        </div>

                        {selectedDeckMeta?.oshi && <span className="badge">Oshi: {selectedDeckMeta.oshi}</span>}

                        {selectedDeckMeta?.color && (
                        <span
                            className="badge"
                            style={{
                            background: `${selectedDeckMeta.color}22`,
                            border: `1px solid ${selectedDeckMeta.color}55`,
                            color: "var(--text)",
                            }}
                        >
                            {selectedDeckMeta.color}
                    ...
                    <div className="muted" style={{ marginTop: 6 }}>
                        ...
                        {mode === "month" ? selectedYM : mode === "lastN" ? `Últimos ${lastN}` : "Torneo"}
                        </span>
                    </div>
                    </div>
                </section>
            ...
            <section className="card" style={{ marginTop: 12 }}>
                ...
                <div className="row" style={{ marginTop: 10, flexWrap: "wrap" }}>
                    ...
                    <div className="muted" style={{ alignSelf: "end", display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                        {selectedDeck ? (
                            <>
                    ...
                    <div className="muted" style={{ alignSelf: "end" }}>
                        {opponentDeck ? (
                            <span style={{ display: "inline-flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                                ...
                                {opponentDeckIcon && (
                                    <img
                                    src={withBase(opponentDeckIcon)}
                                    alt=""
                                    style={{ width: 18, height: 18, borderRadius: 7, objectFit: "cover" }}
                                    />
                                )}
                                {opponentDeckName}
                                </span>
